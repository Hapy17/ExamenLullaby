...
class Harmonic_Theme_Plugin_Enhancements {
	...
	function get_theme_dependencies() {
		...
		if ( current_theme_supports( 'jetpack-content-options' ) ) :
			$dependencies['content-options'] = array(
				'name' => esc_html__( 'Content Options', 'harmonic' ),
	...
	function admin_notices() {
		...
		if ( get_user_meta( get_current_user_id(), 'harmonic_jetpack_admin_notice', true ) === 'dismissed' ) {
			return;
		}
...
function enqueue_scripts() {
	...
	if ( is_admin() && get_user_meta( get_current_user_id(), 'harmonic_jetpack_admin_notice', true ) !== 'dismissed' ) {

		// Adds our JS file to the queue that WordPress will load
		wp_enqueue_script( 'harmonic_jetpack_admin_script', get_template_directory_uri() . '/inc/plugin-enhancements.js', array( 'jquery' ), '20160624', true );

		// Make some data available to our JS file
		wp_localize_script( 'harmonic_jetpack_admin_script', 'harmonic_jetpack_admin', array(
			'harmonic_jetpack_admin_nonce' => wp_create_nonce( 'harmonic_jetpack_admin_nonce' ),
		));
	}
}
...
function dismiss_admin_notice() {
	...
	if ( ! isset( $_POST['harmonic_jetpack_admin_nonce'] ) || ! wp_verify_nonce( $_POST['harmonic_jetpack_admin_nonce'], 'harmonic_jetpack_admin_nonce' ) ) {
		wp_die( __( 'Your request failed permission check.', 'harmonic' ) );
	}
	// Store the user's dimissal so that the notice doesn't show again
	update_user_meta( get_current_user_id(), 'harmonic_jetpack_admin_notice', 'dismissed' );
	// Send success message
	wp_send_json( array(
		'status' => 'success',
		'message' => __( 'Your request was processed. See ya!', 'harmonic' )
	) );
}
add_action( 'wp_ajax_harmonic_jetpack_admin_notice', 'dismiss_admin_notice' );
